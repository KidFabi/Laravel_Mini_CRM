...
@section('content')
    <form action="{{ route('employees.store') }}" method="POST">
        ...
        <div class="form-group mt-3">
            ...
            <input type="number" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" value="{{ old('phone') }}" placeholder="{{ __('Phone') }}" minlength="10" maxlength="10">
            @error ('phone')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <button type="submit" class="btn btn-primary mt-3">{{ __('Create an employee') }}</button>
    </form>
@endsection